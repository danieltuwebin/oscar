...
switch ($_GET["op"]) {
	...
	case 'listar':
		...
		while ($reg = $rspta->fetch_object()) {
			...
			if ($reg->estado == '3') {
				$estadob = '<span class="label bg-red">Rechazo</span>';
			}
			if ($reg->estado == '4') {
				$estadob = '<span class="label label-default">Anul. pend.</span>';
			}
			if ($reg->estado == '5') {
				$estadob = '<span class="label bg-orange">Anul. Env.</span>';
			}
			if ($reg->estado == '6') {
				$estadob = '<span class="label bg-green">An. Acep.</span>';
			}
